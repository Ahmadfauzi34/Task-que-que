...
fail() {
  printf 'termux heterogeneous worker smoke error: %s\n' "$*" >&2
  exit 1
}
...
TASK_QUEUE_BUN_BIN="$BUN_BIN" \
  bash tests/reference-heterogeneous-workers-smoke.sh

printf '\nPhysical Termux heterogeneous worker proof\n'
...
printf '\nTermux heterogeneous workers: OK\n'
