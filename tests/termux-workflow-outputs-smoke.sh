...
fail() {
  printf 'termux workflow outputs smoke error: %s\n' "$*" >&2
  exit 1
}
...
TASK_QUEUE_BUN_BIN="$BUN_BIN" \
  bash tests/reference-workflow-outputs-smoke.sh

printf '\nPhysical Termux declared workflow outputs proof\n'
...
printf 'public workflow result          : EXECUTED\n'
printf 'declared output declassification: EXECUTED\n'
printf 'undeclared child fields         : HIDDEN\n'
printf 'loopback child projection       : PRESERVED\n'
printf 'invalid output authority        : REJECTED\n'
printf '\nTermux declared workflow outputs: OK\n'
