...
fail() {
  printf 'termux lease-loss cancellation smoke error: %s\n' "$*" >&2
  exit 1
}
...
"$BUN_BIN" test workers/document-bun/tests/lease-loss-cancellation.test.ts
...
printf '\nPhysical Termux cooperative lease-loss cancellation proof\n'
...
printf 'Bun generic worker              : EXECUTED\n'
printf 'rejected fenced heartbeat       : OBSERVED\n'
printf 'active handler abort signal     : DELIVERED\n'
printf 'post-fence fail transition      : SUPPRESSED\n'
printf 'post-fence complete transition  : SUPPRESSED\n'
printf 'existing fenced worker flow     : PRESERVED\n'
printf '\nTermux cooperative lease-loss cancellation: OK\n'
