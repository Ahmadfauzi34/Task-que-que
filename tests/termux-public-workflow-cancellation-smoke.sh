...
fail() {
  printf 'termux public workflow cancellation smoke error: %s\n' "$*" >&2
  exit 1
}
...
[ "$ACTUAL_WORKER_SHA" = "$EXPECTED_WORKER_SHA" ] || fail "worker broker binary checksum does not match reviewed artifact"

TASK_QUEUE_RUST_BIN="$QUEUE_BIN" \
TASK_QUEUE_WORKER_BIN="$BROKER_BIN" \
TASK_QUEUE_BUN_BIN="$BUN_BIN" \
  bash tests/reference-public-workflow-cancellation-smoke.sh

printf '\nPhysical Termux public workflow cancellation proof\n'
...
printf 'public cancellation API         : EXECUTED\n'
printf 'running parent fence            : REVOKED\n'
printf 'heartbeat after revocation      : REJECTED\n'
printf 'workflow AbortSignal            : DELIVERED\n'
printf 'post-cancel child enqueue       : NOT STARTED\n'
printf 'cancelled result artifact       : NOT WRITTEN\n'
printf 'public result disclosure        : NONE\n'
printf 'existing fenced worker flow     : PRESERVED\n'
printf '\nTermux public workflow cancellation: OK\n'
