...
fail() {
  printf 'termux result projection smoke error: %s\n' "$*" >&2
  exit 1
}
...
printf '\nPhysical Termux fenced result projection proof\n'
...
printf 'stale projection                : REJECTED\n'
printf 'valid fenced projection         : PERSISTED\n'
printf 'legacy empty completion         : COMPATIBLE\n'
printf 'Bun document/hash projections   : EXECUTED\n'
printf '\nTermux fenced result projection: OK\n'
