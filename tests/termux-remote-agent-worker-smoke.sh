...
fail() {
  printf 'termux remote agent worker smoke error: %s\n' "$*" >&2
  exit 1
}
...
TASK_QUEUE_BUN_BIN="$BUN_BIN" \
  bash tests/reference-remote-agent-worker-smoke.sh

printf '\nPhysical Termux remote agent worker proof\n'
...
printf 'bounded mock agent endpoint      : EXECUTED\n'
printf '\nTermux remote agent worker: OK\n'
