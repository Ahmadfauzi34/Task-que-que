...
fail() {
  printf 'termux workflow worker smoke error: %s\n' "$*" >&2
  exit 1
}
...
TASK_QUEUE_BUN_BIN="$BUN_BIN" \
  bash tests/reference-workflow-worker-smoke.sh

printf '\nPhysical Termux workflow core proof\n'
...
printf 'Bun workflow worker             : EXECUTED + RESTARTED\n'
printf 'Bun cpu worker                  : EXECUTED\n'
printf 'Bun vector worker               : EXECUTED\n'
printf 'Bun remote agent worker         : EXECUTED\n'
printf '\nTermux workflow core: OK\n'
