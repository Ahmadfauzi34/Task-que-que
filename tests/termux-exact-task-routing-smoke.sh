...
fail() {
  printf 'termux exact task routing smoke error: %s\n' "$*" >&2
  exit 1
}
...
printf '\nPhysical Termux exact task-name routing proof\n'
...
printf 'exact task advertisement        : REQUIRED\n'
printf 'same-type unsupported routing   : REJECTED\n'
printf 'Bun handler advertisement       : EXECUTED\n'
printf 'existing fenced worker flow     : PRESERVED\n'
printf '\nTermux exact task-name routing: OK\n'
