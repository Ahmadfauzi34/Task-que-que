...
impl RobustSinkhornQueue {
    ...
    pub fn enqueue(
        ...
        DatabaseManager::execute_with_retry(&self.db_path, move |conn| {
            ...
            // PATCH 1A:
            // Tidak memakai `RETURNING id`.
            // Sebagai gantinya pakai `last_insert_rowid()` yang kompatibel luas.
            tx.execute(
                "INSERT INTO tasks
                    (
                        task_name,
    ...
    pub fn dispatch_batch(
        ...
        DatabaseManager::execute_with_retry(&self.db_path, move |conn| {
            ...
            let rows = {
                ...
                stmt.query_map(params![now, total_available], |row| {
                    Ok(PendingTask {
                        id: row.get(0)?,
                        name: row.get(1)?,
                        task_type: row.get(2)?,
                ...
                .collect::<std::result::Result<Vec<PendingTask>, _>>()?
            };
            ...
            // PATCH 1B:
            // Update tidak dieksekusi sebagai autocommit satu-satu di luar kontrol.
            // Semua update dibungkus dalam satu transaksi + prepared statement.
            // Ini menjaga write lock SQLite tetap singkat dan konsisten.
            {
...
fn sinkhorn_knopp_log_domain(
    ...
    // Normalisasi baris agar setiap task memiliki massa probabilitas yang konsisten.
    // Ini juga membuat plan lebih aman dipakai oleh bounded rounding.
    for i in 0..n {
...
fn round_transport_plan_bounded(plan: &Array2<f64>, capacities: &[i64]) -> Vec<i32> {
    ...
    // Greedy pass: ambil pasangan task->worker dengan skor transport tertinggi.
    ...
    // Fallback pass:
    // jika greedy pertama belum menutup semua task, isi capacity yang tersisa.
    for i in 0..n {
        if assigned[i] {
            continue;
        }
...
```rust
use robust_sinkhorn_queue::{QueueResult, RobustSinkhornQueue, WorkerDescriptor};

fn main() -> QueueResult<()> {
    let queue = RobustSinkhornQueue::new("queue.db");
    queue.ensure_schema()?;

    queue.enqueue_simple(
        "train-model-a",
        "gpu",
        r#"{"batch_size": 64, "model": "resnet"}"#,
    )?;
    ...
    let workers = vec![
        WorkerDescriptor {
            worker_id: "gpu-01".into(),
            worker_type: "gpu".into(),
            capacity: 4,
            available_slots: 2,
        },
        WorkerDescriptor {
            worker_id: "cpu-01".into(),
    ...
    let recovered = queue.recover_expired_leases()?;
    println!("recovered expired leases: {recovered}");

    let dispatched = queue.dispatch_batch_defaults(&workers)?;
    println!("dispatched: {dispatched:#?}");

    if let Some(task) = queue.claim_task("gpu-01")? {
        println!("claimed task: {:?}", task);

        queue.heartbeat(task.id, "gpu-01", 30.0)?;
        queue.complete_task(task.id, "gpu-01")?;
    }
...
### 1. Patch Python `RETURNING id` sudah dipetakan
...
Jadi tetap kompatibel dengan SQLite versi lama.

---

### 2. Patch batch update sudah dipetakan

Python memakai `executemany` agar tidak menahan write lock terlalu lama.

Versi Rust di sini memakai:

- satu transaksi `IMMEDIATE`,
- satu prepared statement,
- loop update di dalam transaksi tersebut.

Efeknya serupa: update tidak menjadi autocommit liar per statement di luar transaksi.

---

### 3. Rust tidak punya default argument

Karena itu saya tambahkan helper:

```rust
enqueue_simple(...)
dispatch_batch_defaults(...)
```

Kalau mau, bisa juga diganti builder pattern, tapi bentuk di atas adalah yang paling dekat dengan gaya Python asli.

---

### 4. `sinkhorn_knopp_log_domain` dan `round_transport_plan_bounded` sudah termasuk

Dua fungsi ini tidak ada di potongan Python awal, jadi saya implementasikan sebagai referensi lengkap agar Rust-nya benar-benar bisa jalan.

---

### 5. Versi ini sengaja tidak “minimal”

Ini dibuat sebagai **reference machine**:

- retry otomatis untuk `SQLITE_BUSY` / `SQLITE_LOCKED`,
- jitter backoff,
- WAL best-effort,
- schema bootstrap,
- transaksi eksplisit,
